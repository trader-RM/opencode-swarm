...
Auto-selects architect agent as primary with defensive fallback; validated via Zod schema.

### Help command

`/swarm help [command]` shows categorized command listing grouped by Core, Agent, Config, Diagnostics, Utility.

### Deprecation aliases

9 legacy aliases (doctor, info, list-agents, health, check, clear, diagnosis, config-doctor, evidence-summary) redirect to canonical commands with non-blocking warnings.

### Command aliases infrastructure

Registry supports `aliasOf` + `deprecated` fields with module-load-time validation for circular references and missing targets. `SKIP_ALIASES` is now dynamically generated from registry entries with `aliasOf` instead of a hardcoded list.
...
None. All changes are backward-compatible. The auto-created file is an empty JSON object and has no effect on existing global-config-only setups. Deprecated aliases continue to function with warnings directing to canonical names.
...
- On read-only filesystems or containers without write access to the project
  root, the config file will not be created (non-fatal). The plugin continues normally.
