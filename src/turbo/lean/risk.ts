...
import { isGlobalFile, isProtectedPath } from './conflicts';
...
export function assessTaskRisk(
	...
	// Check for global files first (highest priority)
	const globalFiles = files.filter(isGlobalFile);
	...
	// Check for invalid scope entries
	if (hasInvalidScope) {
		return {
			category: 'invalid-scope',
			reason: 'invalid scope entries',
			files,
		};
	}
	...
	if (!hasDeclaredScope && config.require_declared_scope) {
		return {
			category: 'no-scope',
			reason: 'undeclared scope',
			files,
		};
	}

	// Normal: no risk conditions detected
	return {
		category: 'normal',
		files: [],
	};
