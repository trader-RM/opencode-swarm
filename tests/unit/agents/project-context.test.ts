...
describe('buildProjectContext', () => {
	...
	test('TypeScript project: PROJECT_LANGUAGE populated, sentinel for unresolved fields', async () => {
		...
		// PROJECT_FRAMEWORK still UNRESOLVED in this phase (deferred follow-up).
...
describe('init fail-open behavior', () => {
	test('LANG_BACKEND_DETECTION_TIMEOUT_MS is the documented 2000ms budget', () => {
		// The session-init wrapper in src/index.ts keys off this constant.
		// Changing it requires updating the surrounding `withTimeout` wrap.
		expect(LANG_BACKEND_DETECTION_TIMEOUT_MS).toBe(2000);
	});
