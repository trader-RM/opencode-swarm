 ...
 * `withTimeout(LANG_BACKEND_DETECTION_TIMEOUT_MS = 2000)` and falls open to
...
describe('init fail-open: buildProjectContext under simulated hang', () => {
	...
	test('caller-side withTimeout(2000) bounds a hung pickBackend', async () => {
		...
		// `withTimeout(buildProjectContext(...), 2000, error).catch(() => null)`.
		// We verify that race resolves within 2s rather than hanging forever.
		...
		// Either the buildProjectContext racer resolved (very unlikely
		// when pickBackend hangs), or our 2.5s setTimeout resolved with
		// null. Either way, we must not have hung beyond 3s.
